import numpy as np
import scipy.signal as signal
import matplotlib.pyplot as plt
import scipy.io as spio
import math as math

fs = 100.0
ny = 0.5*fs

eeg1c3 = np.loadtxt('Data\eeg1-c3.dat', delimiter = ',')
# ind = np.arange(eeg1c3.size)
# t = ind*(1/fs)
plt.subplot(9,1,1)
plt.plot(eeg1c3)
plt.ylabel('Ref c3')
plt.grid(True)

eeg1c4 = np.loadtxt('Data\eeg1-c4.dat', delimiter = ',')
eeg1f3 = np.loadtxt('Data\eeg1-f3.dat', delimiter = ',')
eeg1f4 = np.loadtxt('Data\eeg1-f4.dat', delimiter = ',')
eeg1o1 = np.loadtxt('Data\eeg1-o1.dat', delimiter = ',')
eeg1o2 = np.loadtxt('Data\eeg1-o2.dat', delimiter = ',')
eeg1p3 = np.loadtxt('Data\eeg1-p3.dat', delimiter = ',')
eeg1p4 = np.loadtxt('Data\eeg1-p4.dat', delimiter = ',')

rf = eeg1c3[504:514]-np.mean(eeg1c3[504:514])
lnrf = len(rf)

corr_c3 = []
det_c3 = []
corr_c4 = []
det_c4 = []

corr_f3 = []
det_f3 = []
corr_f4 = []
det_f4 = []

corr_o1 = []
det_o1 = []
corr_o2 = []
det_o2 = []

corr_p3 = []
det_p3 = []
corr_p4 = []
det_p4 = []


for i in range(len(eeg1c3)-lnrf):
  
  tmp_c3 = eeg1c3[i:i+lnrf]-np.mean(eeg1c3[i:i+lnrf])
  tmp_c4 = eeg1c4[i:i+lnrf]-np.mean(eeg1c4[i:i+lnrf])
  a_crr_c3 = np.dot(tmp_c3,rf)/math.sqrt(np.dot(tmp_c3,tmp_c3)*np.dot(rf,rf))
  a_crr_c4 = np.dot(tmp_c4,rf)/math.sqrt(np.dot(tmp_c4,tmp_c4)*np.dot(rf,rf))
  corr_c3.append(a_crr_c3)
  corr_c4.append(a_crr_c4)
  det_c3.append(a_crr_c3 > 0.5)
  det_c4.append(a_crr_c4 > 0.5)
  
  tmp_f3 = eeg1f3[i:i+lnrf]-np.mean(eeg1f3[i:i+lnrf])
  tmp_f4 = eeg1f4[i:i+lnrf]-np.mean(eeg1f4[i:i+lnrf])
  a_crr_f3 = np.dot(tmp_f3,rf)/math.sqrt(np.dot(tmp_f3,tmp_f3)*np.dot(rf,rf))
  a_crr_f4 = np.dot(tmp_f4,rf)/math.sqrt(np.dot(tmp_f4,tmp_f4)*np.dot(rf,rf))
  corr_f3.append(a_crr_f3)
  corr_f4.append(a_crr_f4)
  det_f3.append(a_crr_f3 > 0.5)
  det_f4.append(a_crr_f4 > 0.5)
  
  tmp_o1 = eeg1o1[i:i+lnrf]-np.mean(eeg1o1[i:i+lnrf])
  tmp_o2 = eeg1o2[i:i+lnrf]-np.mean(eeg1o2[i:i+lnrf])
  a_crr_o1 = np.dot(tmp_o1,rf)/math.sqrt(np.dot(tmp_o1,tmp_o1)*np.dot(rf,rf))
  a_crr_o2 = np.dot(tmp_o2,rf)/math.sqrt(np.dot(tmp_o2,tmp_o2)*np.dot(rf,rf))
  corr_o1.append(a_crr_o1)
  corr_o2.append(a_crr_o2)
  det_o1.append(a_crr_o1 > 0.5)
  det_o2.append(a_crr_o2 > 0.5)
  
  tmp_p3 = eeg1p3[i:i+lnrf]-np.mean(eeg1p3[i:i+lnrf])
  tmp_p4 = eeg1p4[i:i+lnrf]-np.mean(eeg1p4[i:i+lnrf])
  a_crr_p3 = np.dot(tmp_p3,rf)/math.sqrt(np.dot(tmp_p3,tmp_p3)*np.dot(rf,rf))
  a_crr_p4 = np.dot(tmp_p4,rf)/math.sqrt(np.dot(tmp_p4,tmp_p4)*np.dot(rf,rf))
  corr_p3.append(a_crr_p3)
  corr_p4.append(a_crr_p4)
  det_p3.append(a_crr_p3 > 0.5)
  det_p4.append(a_crr_p4 > 0.5)

plt.subplot(9,1,2)
plt.plot(corr_c3)
plt.plot(det_c3)
plt.ylabel('Corr c3')
plt.grid(True)

plt.subplot(9,1,3)
plt.plot(corr_c4)
plt.plot(det_c4)
plt.ylabel('Corr c4')
plt.grid(True)

plt.subplot(9,1,4)
plt.plot(corr_f3)
plt.plot(det_f3)
plt.ylabel('Corr f3')
plt.grid(True)

plt.subplot(9,1,5)
plt.plot(corr_f4)
plt.plot(det_f4)
plt.ylabel('Corr f4')
plt.grid(True)

plt.subplot(9,1,6)
plt.plot(corr_o1)
plt.plot(det_o1)
plt.ylabel('Corr o1')
plt.grid(True)

plt.subplot(9,1,7)
plt.plot(corr_o2)
plt.plot(det_o2)
plt.ylabel('Corr o2')
plt.grid(True)

plt.subplot(9,1,8)
plt.plot(corr_p3)
plt.plot(det_p3)
plt.ylabel('Corr p3')
plt.grid(True)

plt.subplot(9,1,9)
plt.plot(corr_p4)
plt.plot(det_p4)
plt.ylabel('Corr p4')
plt.grid(True)

plt.show()







